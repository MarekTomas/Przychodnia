...
public class PacjController {
	...
	@FXML
	//wyœwietlanie wszystkich dostêpnych wizyt
	...
	void buttonClickRezerwuj(MouseEvent event) throws SQLException, IOException {
		...
		String rezer = "insert into zarezerwowane(id_v,imie, nazwisko, specjalizacja, day_v, time_v,pesel,id_d)values('"
				...
				+ czas_selected + "','"+ pas +"','"+ id_d_selected+"');";
	...
	@FXML
	//Wyœwietlanie zarezerwowanych wizyt
	void buttonClickZarezWyswietl(MouseEvent event) throws SQLException {
		...
		ResultSet rezs = conn.createStatement().executeQuery(
				"select id_v,imie, nazwisko, specjalizacja, day_v, time_v, pesel,id_d from zarezerwowane where pesel ="+"'"+pas+"';");
		while (rezs.next()) {
			rezer.add(new Rezervation(rezs.getInt(1), rezs.getString(2), rezs.getString(3), rezs.getString(4), rezs.getDate(5),
					rezs.getTime(6), rezs.getString(7),rezs.getInt(8)));
	...
	void ButtonClickZarezOdwolaj(MouseEvent event) throws SQLException {
		...
		String zwrezer = "insert into visits(id_v,day_v,time_v,id_d)values('"+ id_selected + "','" + date_selected + "','" + time_selected +"','"+ id_d_selected + "');";
		
		PreparedStatement pszwrezer = conn.prepareStatement(zwrezer);
		pszwrezer.executeUpdate();
		buttonClickZarezWyswietl(event);
		System.out.println(zwrezer);
